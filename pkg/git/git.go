...
func (g *binaryGitter) checkoutGithub(repo, branchOrCommit string) error {
	...
	doCheckout := func(branch string) error {
		repoFolder := strings.ReplaceAll(strings.ReplaceAll(repo, "/", "-"), "https:--", "")
		g.folder = filepath.Join(os.TempDir(),
			repoFolder+"-"+shortid.MustGenerate())

		orgRepo := strings.TrimPrefix(repo, "https://github.com/")

		// https://api.github.com/repos/m3o/m3o/zipball/main
		url := fmt.Sprintf("https://api.github.com/repos/%v/zipball/%v", orgRepo, branch)
		...
		if resp.StatusCode > 399 {
			log.Errorf("Error retrieving zip %s %v", resp.Status, resp.StatusCode)
			return fmt.Errorf("Error retrieving zip %s", resp.Status)
		}
	...
	if branchOrCommit == "latest" {
		err := doCheckout("")
		if err == errNotFound {
			return doCheckout(branchOrCommit)
		}
		return err
	} else {
		return doCheckout(branchOrCommit)

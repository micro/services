...
func urlPath(microServiceName, protoServiceName, methodName string) string {
	if strings.ToLower(microServiceName) == strings.ToLower(protoServiceName) {
		return fmt.Sprintf("/v1/%s/%s", microServiceName, methodName)
	}
	return fmt.Sprintf("/v1/%s/%s/%s", microServiceName, protoServiceName, methodName)
}
